...
<section class="content-header">
    <h1>@lang('menu/title.management')</h1>
    <ol class="breadcrumb">
        <li>
        ...
        <li><a href="#"> @lang('menu/title.menu')</a></li>
        <li class="active">@lang('menu/title.menulist')</li>
    </ol>
</section>
                ...
                <div class="panel-heading clearfix">
                    <div class="container-fluid">
                        <div class="row">
                            ...
                            <div class="col-xs-6 col-md-6"> 
                                <div class="pull-right">
									@if ($user->hasAccess(['menu.data.quickcreate']))
									...
									@endif
									@if ($user->hasAccess(['menu.data.quickrename']))
									...
									@endif
									@if ($user->hasAccess(['menu.data.quickremove']))
									...
									@endif
									@if ($user->hasAccess(['menu.data.quickcreate']))
										<a href="{{ URL::to('admin/menu/create') }}" class="btn btn-sm btn-default"><span class="glyphicon glyphicon-plus"></span> @lang('button.create')</a>
									@endif
                                </div>
                            </div>
                        </div>
...
@section('footer_scripts')
    ...
    <script type="text/javascript">
        function quick_create() {
            var ref = $('#menu_items').jstree(true),
        ...
        function deleteItem() {
            ...
            $.ajax({
                type: 'GET', 
                url: '{!! route('menu.data.quickremove') !!}', 
                data: {id: id}, 
                dataType: 'json',
        ...
        $(document).ready(function() {
            ...
            $('#tree_search').keyup(function () {
                ...
                to = setTimeout(function () {
                    ...
                    $('#menu_items').jstree(true).search(v);
                }, 250);
            });
            ...
            $('#menu_items').jstree({
				...
				'contextmenu': {         
					"items": function($node) {
						var tree = $("#menu_items").jstree(true);
						return {
							"Open": {
                ...
                if(isNaN(data.node.id)) {
                    $.ajax({
                        type: 'GET', 
                        url: '{!! route('menu.data.quickcreate') !!}', 
                        data: {text: text, parent: parentId, position: position }, 
                        dataType: 'json',
                        success: function (data) {
                            $('#menu_items').jstree(true).refresh();
                        }
                    });
                } else {
                    $.ajax({
                        type: 'GET', 
                        url: '{!! route('menu.data.quickrename') !!}', 
			...
			$("#menu_items").delegate("a","dblclick", function(e) {
				var ref = $('#menu_items').jstree(true);
				sel = ref.get_selected();
				var id = sel[0];
    ...
    <div class="modal fade" id="delete_confirm" tabindex="-1" role="dialog" aria-labelledby="menu_delete_confirm_title" aria-hidden="true">
